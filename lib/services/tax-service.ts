...
export const fetchSellTransactions = async (): Promise<TableTransaction[]> => {
  try {
    ...
    return transactions
      .filter((transaction) => !transaction.filedWithIRS)
...
export const isSellComplete = (
  ...
  const totalSellAmount =
    otherSellsOfSameAsset.reduce((sum, tx) => sum + tx.amount, 0) + sellTransaction.amount;
...
export const generateSellReportSummaries = (
  ...
  selectedSellTransactions.forEach((sellId) => {
    ...
    const otherSellsOfSameAsset = sellsByAsset[sellTransaction.asset].filter(
      (tx) => tx.id !== sellId
    );

    // Filter to just the ones associated with this sell
    const associatedBuyTransactions = allBuyTransactions.filter((tx) => buyIds.includes(tx.id));

    // Calculate profit
    const buyTotal = associatedBuyTransactions.reduce((sum, tx) => sum + tx.total, 0);
