...
export function BuyTransactions({
  ...
  useEffect(() => {
    ...
    const fetchMissingSelectedTransactions = async () => {
      ...
      const currentPageSelectedTxs = tableTransactions.filter((tx) =>
  ...
  const isLongTerm =
    selectedTxs.length > 0 ? selectedTxs.every((tx) => tx.term === 'Long Term') : false;
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow>
                <TableCell colSpan={7} className="text-center">
                  No buy transactions found for {asset}
                </TableCell>
              </TableRow>
            ) : (
