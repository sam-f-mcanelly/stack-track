...
export const SellTransactionTable: React.FC<SellTransactionTableProps> = ({
  ...
  const isCurrentPageAllSelected = () => {
    ...
    return (
      currentItems.length > 0 && currentItems.every((tx) => selectedTransactions.includes(tx.id))
    );
  };
          ...
          {isLoading ? (
              ...
              <TableCell colSpan={8} className="h-24">
                <div className="flex flex-col items-center justify-center py-6 text-center">
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-500">
                    There are no unreported sell transactions for {year}
                  </p>
                </div>
              </TableCell>
            </TableRow>
            ...
            currentItems.map((tx) => (
              <TableRow
                key={tx.id}
                className={cn(
                  selectedTransactions.includes(tx.id) ? 'bg-green-50 dark:bg-green-900/20' : '',
              ...
              >
                ...
                <TableCell className="py-2 font-medium text-sm">{formatDate(tx.date)}</TableCell>
                <TableCell className="py-2">
