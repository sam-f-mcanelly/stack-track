...
import {
  ...
  TaxType,
  UsedBuyTransaction,
} from '@/lib/models/backend/tax/tax';
...
export function ReportSummary({
  ...
  const totalGain = Object.values(taxReports).reduce(
    (sum, report) => sum + (report?.gain?.amount || 0),
    0
  );

  const totalProceeds = Object.values(taxReports).reduce(
    (sum, report) => sum + (report?.proceeds?.amount || 0),
    0
  );
    ...
    <Card className="mb-4 overflow-hidden">
      <CardHeader className="py-4">
        <div className="flex justify-between items-center">
          <div>
            ...
            {selectedSellTransactions.length > 0 &&
              taxReports &&
              Object.keys(taxReports).length > 0 && (
        ...
        <CardDescription className="text-xs mt-1">
          Summary of transactions selected for the tax report
        </CardDescription>
      </CardHeader>
        ...
        {selectedSellTransactions.length === 0 ? (
          <div className="text-center py-4 bg-gray-50 dark:bg-gray-800 rounded-lg border border-dashed border-gray-300 dark:border-gray-700">
            <p className="text-slate-500 dark:text-slate-400 text-sm">
              Select a sell transaction to add it to the report
            </p>
          </div>
        ) : (
          <>
            <div className="space-y-2">
              {currentPageItems.map((sellId) => {
                ...
                const gainPercentage =
                  hasTaxReport && taxReport.costBasis.amount !== 0
                        ...
                        <div className="flex items-center">
                          ...
                          <div>
                            <div className="flex items-center">
                              ...
                              {isNewlyUpdated && (
                                <Badge className="ml-1 text-xs py-0 px-1 h-5" variant="default">
                                  New
                                </Badge>
                              )}
                            </div>
                            <div className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                              {hasTaxReport ? taxReport.sellTransaction.timestampText : sellTx.date}{' '}
                              · {taxMethods[sellId]}
                            </div>
                          </div>
                        </div>
                        <div className="flex flex-col items-end">
                          {hasTaxReport ? (
                            <>
                              ...
                              >
                                {taxReport.gain.amount > 0 ? '+' : ''}
                                {formatValue(taxReport.gain.amount, taxReport.gain.unit)}
                              </Badge>
                            </>
                          ) : (
                            <div className="text-xs">@ ${sellTx.price.toFixed(2)}</div>
                          )}
                        </div>
                      </div>
                    </div>
                              ...
                              <div className="flex items-start">
                                ...
                                <div>
                                  <p className="text-xs text-orange-800 dark:text-orange-300">
                                    ...
                                    {taxReport.uncoveredSellAmount.amount.toFixed(6)}{' '}
                                    {taxReport.uncoveredSellAmount.unit}
                                    {taxReport.uncoveredSellValue && (
                                      <span>
                                        {' '}
                                        (
                                        {formatValue(
                                          taxReport.uncoveredSellValue.amount,
                                          taxReport.uncoveredSellValue.unit
                                        )}
                                        )
                                      </span>
                                    )}
                                    ...
                                    (buyTx: UsedBuyTransaction) => {
                                      ...
                                      return (
                                        <TableRow key={buyTx.transactionId} className="h-7">
                                          ...
                                          <TableCell className="py-1">
                                            {formatValue(
                                              buyTx.costBasis.amount,
                                              buyTx.costBasis.unit
                                            )}
                                          </TableCell>
                                          <TableCell className="py-1">
                                            <Badge
