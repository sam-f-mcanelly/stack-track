...
export function CsvManager({ 
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setStatus({ type: 'error', message: 'Network error occurred. Please check your connection.' });
    } finally {
  ...
  return (
    <Card className="w-full max-w-md mx-auto dark:border-gray-700">
      <CardHeader className="space-y-1 relative overflow-hidden">
        ...
        <CardDescription>
          Upload your CSV files or download transaction data
        </CardDescription>
      </CardHeader>
      <CardContent>
  ...
  return (
    <div className={`relative ${className}`}>
      <motion.svg
        ...
        animate={{ 
          rotateY: 360,
          filter: ["drop-shadow(0 0 8px rgba(247, 147, 26, 0.5))", "drop-shadow(0 0 16px rgba(247, 147, 26, 0.3))", "drop-shadow(0 0 8px rgba(247, 147, 26, 0.5))"]
        }}
        transition={{ 
          rotateY: { 
            repeat: Infinity, 
